<template>
 <TableBaseComponent>
   ...
   <template #tableBody>
     <tr
         v-for="(driver, key) in displayedDrivers"
         :key="key"
     >
       <td
           v-for="(tableColumn, index) in tableColumns"
       ...
       >
          <span>
            {{ driver[tableColumn.key] }}
          </span>
       </td>
       <td class="routes-body-img">
         <ButtonBaseComponent
             class="routes-body-img-button"
             @click-action="toggleModalCard(driver)"
         >
  ...
  <Paginator
    :rows="rowsPerPage"
    :total-records="store.drivers.length"
    @page="onPageChange"
  />
</template>
...
const displayedDrivers = computed(() => {
  ...
  return drivers.value.slice(startIndex, endIndex)
})
